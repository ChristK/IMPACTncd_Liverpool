...
List collect_output(
  ...
  for (int i = 0; i < nrow; i++) { //loop through rows
    ...
    if (aux[1] == -120) aux[1] = (((std::rand() / (float)RAND_MAX) <  0.5) ? -100 : -20);
    if (aux[1] == -103) aux[1] = (((std::rand() / (float)RAND_MAX) <  0.5) ? -3   : -100);
    if (aux[1] ==  -23) aux[1] = (((std::rand() / (float)RAND_MAX) <  0.5) ? -20  : -3);
...
Rcpp::IntegerVector predict_svylr(arma::mat x, arma::uword e, arma::vec y, arma::rowvec z, arma::vec rank) {
  ...
  //x.shed_row(e-1);
  x *= y;
  arma::mat cc = repmat(z, x.n_rows, 1);
  cc.each_col() -= x;
  cc.for_each( [](arma::mat::elem_type& val) { val = Rcpp::stats::plogis_0(val, 1, 0); } );  // C++11! lamda function
  arma::umat cc2 = (cc < repmat(rank, 1, cc.n_cols));
  arma::ucolvec out = sum(cc2,1);

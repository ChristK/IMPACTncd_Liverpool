...
# FV 
POP[, porftvg_sc10 := porftvg]
POP[between(age, 16, 84) & year >= intervention_year2 & rbinom(.N, 1, 0.5) == 1L,
    porftvg_sc10 := as.integer(bound(porftvg_sc10 + 1L, 0, 8))]
setkey(POP, id, year) # Do not remove
POP[, porftvg.cvdlag := shift_byid(porftvg_sc10, cvd.lag.mc, 0, id)] # lag exposure
...
set(POP, NULL, "invited_sc10", 0L)
set(POP, NULL, "dice_coverage",  dice(nrow(POP)))
setkey(POP, id, year) # Do not remove
POP[eligible_sc10 == 1 & year >= intervention_year, 
    invited_sc10 := HC_coverage(id, coverage, dice_coverage)]

# Estimate uptake
set(POP, NULL, "participated_sc10", 0L)
...
POP[invited_sc10 == 1, uptake := fcompress(uptake, .N * hc_annual_uptake), by=year]# Compress weights to givel reach uptake level
POP[invited_sc10 == 1, participated_sc10 := rbinom(.N, 1, uptake)]
...
POP[participated_sc10  == 1,  statins_uptake := bound(prescription_uptake * .N / count_if(cholval >= 5)), by = .(qimd, qrisk_grp)] # adjust weights to reflect only those with high cholesterol
POP[participated_sc10  == 1 & cholval >= 5,
...
POP[participated_sc10  == 1,  antihtn_uptake := bound(prescription_uptake * .N / count_if(omsysval >= 135)), by = .(qimd, qrisk_grp)] # adjust weights to reflect only those with high SBP
POP[participated_sc10  == 1 & omsysval >= 135,
...
set(POP, NULL, "statins_prescribed_sc10", 0L)
POP[participated_sc10  == 1 & cholval >= 5,  statins_prescribed_sc10 := rbinom(.N, 1, statins_uptake)]
set(POP, NULL, "statins_taken_sc10", 0L)
POP[statins_prescribed_sc10  == 1,  statins_taken_sc10 := rbinom(.N, 1, persistence.mc)]

set(POP, NULL, "antihtn_prescribed_sc10", 0L)
POP[participated_sc10  == 1 & omsysval >= 135,  antihtn_prescribed_sc10 := rbinom(.N, 1, antihtn_uptake)]
set(POP, NULL, "antihtn_taken_sc10", 0L)
POP[antihtn_prescribed_sc10  == 1,  antihtn_taken_sc10 := rbinom(.N, 1, persistence.mc)]

set(POP, NULL, "participation_effect_sc10", 0L)
setkey(POP, id, year) # Do not remove
POP[, participation_effect_sc10 := HC_effect(id, participated_sc10, invited_sc10)] # Effect lasts until next invitation
POP[, statins_taken_sc10 := med_effect(id, statins_taken_sc10, participation_effect_sc10)] # Effect lasts until next invitation
POP[, antihtn_taken_sc10 := med_effect(id, antihtn_taken_sc10, participation_effect_sc10)] # Effect lasts until next invitation
#View(POP[year >= intervention_year& eligible_sc10 == 1, .(id, year, invited_sc10, participated_sc10, participation_effect_sc10, statins_prescribed_sc10, statins_taken_sc10)])

# Estimate effect
#cholesterol
POP[, cholval_sc10 := cholval]
POP[statins_taken_sc10  == 1,  cholval_sc10 := cholval * (1 - atorv.eff.mc * adherence.mc)]
setkey(POP, id, year) # Do not remove
POP[, cholval.cvdlag := shift_byid(cholval_sc10, cvd.lag.mc, 3.8, id)] # lag exposure
#SBP
POP[, omsysval_sc10 := omsysval]
POP[antihtn_taken_sc10  == 1,  omsysval := omsysval - (omsysval - 135) * adherence.mc] # Assume that antihtn medication can potentialy achieve sbp 135 for all
...
# Diab probabilities
POP[, bpmed := bpmed_init] #reset for each scenario. not needed for sc10. Here for consistency
POP[antihtn_taken_sc10 == 1L, bpmed := 1L] # feedback loop. Antihtn meds increased prescription increases diab 
...
set(POP, NULL, "htn.incidence_sc10", 0L)
POP[omsysval > 135 | bpmed == 1, htn.incidence_sc10 := 6000L]
...
# Calculate incidence
setkey(POP, id, year)
POP[, diabtotr_sc10 := diabtotr_init]
set(POP, NULL, "chd.incidence_sc10", 0L)
set(POP, NULL, "stroke.incidence_sc10", 0L)
POP[year >= 0, c("diabtotr_sc10", "chd.incidence_sc10", "stroke.incidence_sc10") :=
...
POP[, diabtotr_sc10 := factor_(diabtotr_sc10)]

# Calculate mortality
set(POP, NULL, "death.tob.rr", 1)
POP[cigst1.cvdlag == "4", death.tob.rr := smokriskofdeath]
set(POP, NULL, "death.diab.rr", 1)
POP[, diabtotr.cvdlag := shift_byid(diabtotr_sc10, cvd.lag.mc, 1L, id, 1:2)] # lag exposure
...
POP[, `:=` (chd.mortality_sc10 = 0L,
            stroke.mortality_sc10 = 0L,
            other.mortality_sc10 = 0L)]
setkey(POP, id, year)
POP[year >= 0 & chd.incidence_sc10 > 0,
    chd.mortality_sc10 := mortality_events(pr_chd_death, dice_chd_death, id, -20L)]
POP[year >= 0 & stroke.incidence_sc10 > 0,
    stroke.mortality_sc10 := mortality_events(pr_stroke_death, dice_stroke_death,
                                             id, -3L)]
POP[year >= 0,
    other.mortality_sc10 := mortality_events(pr_other_death, dice_other_death,
                                            id, -100L)]
...
fwrite_safe(summary_RF_fn(RF, nam, "sc10"),         output.dir("sc_RF.csv"),      append = TRUE)
fwrite_safe(summary_RF_fn(RF, nam, "sc10", "qimd"), output.dir("sc_RF_qimd.csv"), append = TRUE)
fwrite_safe(summary_RF_fn(RF, nam, "sc10", "sex"),  output.dir("sc_RF_sex.csv"),  append = TRUE)
fwrite_safe(summary_RF_fn(RF, nam, "sc10", "age"),  output.dir("sc_RF_age.csv"),  append = TRUE)

# shape output
out <- mask_age(out)

fwrite_safe(summary_fn(out, nam, "sc10"),         output.dir("sc_results.csv"),      append = TRUE)
fwrite_safe(summary_fn(out, nam, "sc10", "qimd"), output.dir("sc_results_qimd.csv"), append = TRUE)
fwrite_safe(summary_fn(out, nam, "sc10", "sex"),  output.dir("sc_results_sex.csv"),  append = TRUE)
fwrite_safe(summary_fn(out, nam, "sc10", "age"),  output.dir("sc_results_age.csv"),  append = TRUE)

POP[, grep("_sc10", names(POP), value = T) := NULL]
rm(out, nam, tt, x, RF)

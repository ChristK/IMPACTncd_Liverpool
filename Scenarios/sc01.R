...
set(POP, NULL, "invited_sc01", 0L)
set(POP, NULL, "dice_coverage",  dice(nrow(POP)))
setkey(POP, id, year) # Do not remove
POP[eligible_sc01 == 1 & year >= intervention_year, 
    invited_sc01 := HC_coverage(id, coverage, dice_coverage)]

# Estimate uptake
set(POP, NULL, "participated_sc01", 0L)
set(POP, NULL, "dice_uptake",  dice(nrow(POP)))
POP[uptake_weights, `:=` (uptake = weights),
    on = c("agerange", "sex", "qimd", "qrisk_grp")]
POP[invited_sc01 == 1 & between(year, intervention_year, intervention_year2-1L),
    uptake := hc_annual_uptake * uptake * .N,
    by = .(year)] #bayes theorem numerator
POP[invited_sc01 == 1 & year >= intervention_year2,
...
POP[invited_sc01 == 1, participated_sc01 := rbinom(.N, 1, uptake)]
...
POP[participated_sc01  == 1,  statins_uptake :=
...
POP[participated_sc01  == 1 & cholval >= 5,
...
POP[participated_sc01  == 1,  antihtn_uptake := bound(prescription_uptake * .N / count_if(omsysval >= 135)), by = .(qimd, qrisk_grp)] # adjust weights to reflect only those with high SBP
POP[participated_sc01  == 1 & omsysval >= 135,
...
set(POP, NULL, "statins_prescribed_sc01", 0L)
POP[participated_sc01  == 1 & cholval >= 5,  statins_prescribed_sc01 := rbinom(.N, 1, statins_uptake)]
set(POP, NULL, "statins_taken_sc01", 0L)
POP[statins_prescribed_sc01  == 1,  statins_taken_sc01 := rbinom(.N, 1, persistence.mc)]

set(POP, NULL, "antihtn_prescribed_sc01", 0L)
POP[participated_sc01  == 1 & omsysval >= 135,  antihtn_prescribed_sc01 := rbinom(.N, 1, antihtn_uptake)]
set(POP, NULL, "antihtn_taken_sc01", 0L)
POP[antihtn_prescribed_sc01  == 1,  antihtn_taken_sc01 := rbinom(.N, 1, persistence.mc)]

set(POP, NULL, "participation_effect_sc01", 0L)
setkey(POP, id, year) # Do not remove
POP[, participation_effect_sc01 := HC_effect(id, participated_sc01, invited_sc01)] # Effect lasts until next invitation
POP[, statins_taken_sc01 := med_effect(id, statins_taken_sc01, participation_effect_sc01)] # Effect lasts until next invitation
POP[, antihtn_taken_sc01 := med_effect(id, antihtn_taken_sc01, participation_effect_sc01)] # Effect lasts until next invitation
#View(POP[year >= intervention_year& eligible_sc01 == 1, .(id, year, invited_sc01, participated_sc01, participation_effect_sc01, statins_prescribed_sc01, statins_taken_sc01)])

# Estimate effect
#cholesterol
POP[, cholval_sc01 := cholval]
POP[statins_taken_sc01  == 1,  cholval_sc01 := cholval * (1 - atorv.eff.mc * adherence.mc)]
setkey(POP, id, year) # Do not remove
POP[, cholval.cvdlag := shift_byid(cholval_sc01, cvd.lag.mc, 3.8, id)] # lag exposure
#SBP
POP[, omsysval_sc01 := omsysval]
POP[antihtn_taken_sc01  == 1,  omsysval_sc01 := omsysval - (omsysval - 135) * adherence.mc] # Assume that antihtn medication can potentialy achieve sbp 135 for all
POP[(omsysval - omsysval_sc01)<0, omsysval_sc01 := omsysval]
setkey(POP, id, year) # Do not remove
POP[, omsysval.cvdlag := shift_byid(omsysval_sc01, cvd.lag.mc, 115, id)] # lag exposure

#recalculate diabetes prob
# Diab probabilities
POP[, bpmed := bpmed_init] #reset for each scenario. not needed for sc01. Here for consistency
POP[antihtn_taken_sc01 == 1L, bpmed := 1L] # feedback loop. Antihtn meds increased prescription increases diab 
...
set(POP, NULL, "htn.incidence_sc01", 0L)
POP[omsysval_sc01 > 135 | bpmed == 1, htn.incidence_sc01 := 6000L]
...
# Calculate incidence
setkey(POP, id, year)
POP[, diabtotr_sc01 := diabtotr_init]
set(POP, NULL, "chd.incidence_sc01", 0L)
set(POP, NULL, "stroke.incidence_sc01", 0L)
POP[year >= 0, c("diabtotr_sc01", "chd.incidence_sc01", "stroke.incidence_sc01") :=
...
POP[, diabtotr_sc01 := factor_(diabtotr_sc01)]

# Calculate mortality
set(POP, NULL, "death.tob.rr", 1)
POP[cigst1.cvdlag == "4", death.tob.rr := smokriskofdeath]
set(POP, NULL, "death.diab.rr", 1)
POP[, diabtotr.cvdlag := shift_byid(diabtotr_sc01, cvd.lag.mc, 1L, id, 1:2)] # lag exposure
...
POP[, `:=` (chd.mortality_sc01 = 0L,
            stroke.mortality_sc01 = 0L,
            other.mortality_sc01 = 0L)]
setkey(POP, id, year)
POP[year >= 0 & chd.incidence_sc01 > 0,
    chd.mortality_sc01 := mortality_events(pr_chd_death, dice_chd_death, id, -20L)]
POP[year >= 0 & stroke.incidence_sc01 > 0,
    stroke.mortality_sc01 := mortality_events(pr_stroke_death, dice_stroke_death,
                                             id, -3L)]
POP[year >= 0,
    other.mortality_sc01 := mortality_events(pr_other_death, dice_other_death,
                                            id, -100L)]
...
fwrite_safe(summary_RF_fn(RF, nam, "sc01"),         output.dir("sc_RF.csv"),      append = TRUE)
fwrite_safe(summary_RF_fn(RF, nam, "sc01", "qimd"), output.dir("sc_RF_qimd.csv"), append = TRUE)
fwrite_safe(summary_RF_fn(RF, nam, "sc01", "sex"),  output.dir("sc_RF_sex.csv"),  append = TRUE)
fwrite_safe(summary_RF_fn(RF, nam, "sc01", "age"),  output.dir("sc_RF_age.csv"),  append = TRUE)

# shape output
out <- mask_age(out)

fwrite_safe(summary_fn(out, nam, "sc01"),         output.dir("sc_results.csv"),      append = TRUE)
fwrite_safe(summary_fn(out, nam, "sc01", "qimd"), output.dir("sc_results_qimd.csv"), append = TRUE)
fwrite_safe(summary_fn(out, nam, "sc01", "sex"),  output.dir("sc_results_sex.csv"),  append = TRUE)
fwrite_safe(summary_fn(out, nam, "sc01", "age"),  output.dir("sc_results_age.csv"),  append = TRUE)

POP[, grep("_sc01", names(POP), value = T) := NULL]
rm(out, nam, tt, x, RF)

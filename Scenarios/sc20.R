...
# table 8
# BMI 
POP[ssb.tax.effect.mc, on = c("age", "qimd"), bmival_sc20 := bmival + effect]
POP[!between(age, 16, 84) | year < intervention_year2, bmival_sc20 := bmival]
setkey(POP, id, year) # Do not remove
POP[, bmival.cvdlag := shift_byid(bmival_sc20, cvd.lag.mc, 20, id)] # lag exposure
...
# FV 
POP[, porftvg_sc20 := porftvg]
POP[between(age, 16, 84) & year >= intervention_year2 & rbinom(.N, 1, 0.5) == 1L,
    porftvg_sc20 := as.integer(bound(porftvg_sc20 + 1L, 0, 8))]
setkey(POP, id, year) # Do not remove
POP[, porftvg.cvdlag := shift_byid(porftvg_sc20, cvd.lag.mc, 0, id)] # lag exposure
...
POP[, cigst1_sc20 := cigst1]
POP[between(age, 20, 84) & year >= intervention_year2 & cigst1=="4",
    hh := as.numeric(rbinom(.N, 1, hh))]
POP[hh == 1, cigst1_sc20 := "3"]

setkey(POP, id, year) # Do not remove
POP[, cigst1.cvdlag := shift_byid(cigst1_sc20, cvd.lag.mc, 1L, id, 1:4)] # lag exposure
# needed for QRisk and QDrisk
POP[, smoke_cat_sc20 := smoke_cat]
POP[cigst1_sc20 == "3", smoke_cat_sc20 := 1L]
POP[, hh := NULL]
...
#SBP
POP[salt.reform.effect.mc, on = c("year", "agegroup", "sex", "qimd"), omsysval_sc20 := omsysval + i.effect] 
POP[!between(age, 20, 84) | year < intervention_year2, omsysval_sc20 := omsysval] # 0.8 mmHg reduction from salt policies
...
set(POP, NULL, "invited_sc20", 0L)
set(POP, NULL, "dice_coverage",  dice(nrow(POP)))
setkey(POP, id, year) # Do not remove
POP[eligible_sc20 == 1 & year >= intervention_year, 
    invited_sc20 := HC_coverage(id, coverage, dice_coverage)]

# Estimate uptake
set(POP, NULL, "participated_sc20", 0L)
set(POP, NULL, "dice_uptake",  dice(nrow(POP)))
POP[uptake_weights, `:=` (uptake = weights),
    on = c("agerange", "sex", "qimd", "qrisk_grp")]
POP[invited_sc20 == 1 & between(year, intervention_year, intervention_year2-1L),
    uptake := hc_annual_uptake * uptake * .N,
    by = .(year)] #bayes theorem numerator
POP[invited_sc20 == 1 & year >= intervention_year2,
...
POP[invited_sc20 == 1, participated_sc20 := rbinom(.N, 1, uptake)]
...
POP[participated_sc20  == 1,  statins_uptake :=
...
POP[participated_sc20  == 1 & cholval >= 5,
...
POP[participated_sc20  == 1,  antihtn_uptake := bound(prescription_uptake * .N / count_if(omsysval >= 135)), by = .(qimd, qrisk_grp)] # adjust weights to reflect only those with high SBP
POP[participated_sc20  == 1 & omsysval >= 135,
...
set(POP, NULL, "statins_prescribed_sc20", 0L)
POP[participated_sc20  == 1 & cholval >= 5,  statins_prescribed_sc20 := rbinom(.N, 1, statins_uptake)]
set(POP, NULL, "statins_taken_sc20", 0L)
POP[statins_prescribed_sc20  == 1,  statins_taken_sc20 := rbinom(.N, 1, persistence.mc)]

set(POP, NULL, "antihtn_prescribed_sc20", 0L)
POP[participated_sc20  == 1 & omsysval >= 135,  antihtn_prescribed_sc20 := rbinom(.N, 1, antihtn_uptake)]
set(POP, NULL, "antihtn_taken_sc20", 0L)
POP[antihtn_prescribed_sc20  == 1,  antihtn_taken_sc20 := rbinom(.N, 1, persistence.mc)]

set(POP, NULL, "participation_effect_sc20", 0L)
setkey(POP, id, year) # Do not remove
POP[, participation_effect_sc20 := HC_effect(id, participated_sc20, invited_sc20)] # Effect lasts until next invitation
POP[, statins_taken_sc20 := med_effect(id, statins_taken_sc20, participation_effect_sc20)] # Effect lasts until next invitation
POP[, antihtn_taken_sc20 := med_effect(id, antihtn_taken_sc20, participation_effect_sc20)] # Effect lasts until next invitation
#View(POP[year >= intervention_year& eligible_sc20 == 1, .(id, year, invited_sc20, participated_sc20, participation_effect_sc20, statins_prescribed_sc20, statins_taken_sc20)])

# Estimate effect
#cholesterol
POP[, cholval_sc20 := cholval]
POP[statins_taken_sc20  == 1,  cholval_sc20 := cholval * (1 - atorv.eff.mc * adherence.mc)]
setkey(POP, id, year) # Do not remove
POP[, cholval.cvdlag := shift_byid(cholval_sc20, cvd.lag.mc, 3.8, id)] # lag exposure
#SBP
POP[antihtn_taken_sc20  == 1,  omsysval_sc20 := omsysval_sc20 - (omsysval_sc20 - 135) * adherence.mc] # Assume that antihtn medication can potentialy achieve sbp 135 for all
POP[omsysval < omsysval_sc20, omsysval_sc20 := omsysval]
setkey(POP, id, year) # Do not remove
POP[, omsysval.cvdlag := shift_byid(omsysval_sc20, cvd.lag.mc, 115, id)] # lag exposure

#recalculate diabetes prob DO NOT REMOVE
# Diab probabilities
POP[, bpmed := bpmed_init] #reset for each scenario. not needed for sc20. Here for consistency
POP[antihtn_taken_sc20 == 1L, bpmed := 1L] # feedback loop. Antihtn meds increased prescription increases diab 
set(POP, NULL, "pr_diab_nocvd", 0)
set(POP, NULL, "pr_diab_cvd", 0)
POP[between(age, 25, 84), c("pr_diab_nocvd", "pr_diab_cvd") := QDrisk(age, sex, b_corticosteroids, bpmed, bmival.cvdlag, origin, fh.diab, smoke_cat_sc20, townsend, 1L)]
...
set(POP, NULL, "htn.incidence_sc20", 0L)
POP[omsysval_sc20 > 135 | bpmed == 1, htn.incidence_sc20 := 6000L]
...
# Calculate incidence
setkey(POP, id, year)
POP[, diabtotr_sc20 := diabtotr_init]
set(POP, NULL, "chd.incidence_sc20", 0L)
set(POP, NULL, "stroke.incidence_sc20", 0L)
POP[year >= 0, c("diabtotr_sc20", "chd.incidence_sc20", "stroke.incidence_sc20") :=
...
POP[, diabtotr_sc20 := factor_(diabtotr_sc20)]

# Calculate mortality
set(POP, NULL, "death.tob.rr", 1)
POP[cigst1.cvdlag == "4", death.tob.rr := smokriskofdeath]
set(POP, NULL, "death.diab.rr", 1)
POP[, diabtotr.cvdlag := shift_byid(diabtotr_sc20, cvd.lag.mc, 1L, id, 1:2)] # lag exposure
...
POP[, `:=` (chd.mortality_sc20 = 0L,
            stroke.mortality_sc20 = 0L,
            other.mortality_sc20 = 0L)]
setkey(POP, id, year)
POP[year >= 0 & chd.incidence_sc20 > 0,
    chd.mortality_sc20 := mortality_events(pr_chd_death, dice_chd_death, id, -20L)]
POP[year >= 0 & stroke.incidence_sc20 > 0,
    stroke.mortality_sc20 := mortality_events(pr_stroke_death, dice_stroke_death,
                                             id, -3L)]
POP[year >= 0,
    other.mortality_sc20 := mortality_events(pr_other_death, dice_other_death,
                                            id, -100L)]
...
fwrite_safe(summary_RF_fn(RF, nam, "sc20"),         output.dir("sc_RF.csv"),      append = TRUE)
fwrite_safe(summary_RF_fn(RF, nam, "sc20", "qimd"), output.dir("sc_RF_qimd.csv"), append = TRUE)
fwrite_safe(summary_RF_fn(RF, nam, "sc20", "sex"),  output.dir("sc_RF_sex.csv"),  append = TRUE)
fwrite_safe(summary_RF_fn(RF, nam, "sc20", "age"),  output.dir("sc_RF_age.csv"),  append = TRUE)

# shape output
out <- mask_age(out)

fwrite_safe(summary_fn(out, nam, "sc20"),         output.dir("sc_results.csv"),      append = TRUE)
fwrite_safe(summary_fn(out, nam, "sc20", "qimd"), output.dir("sc_results_qimd.csv"), append = TRUE)
fwrite_safe(summary_fn(out, nam, "sc20", "sex"),  output.dir("sc_results_sex.csv"),  append = TRUE)
fwrite_safe(summary_fn(out, nam, "sc20", "age"),  output.dir("sc_results_age.csv"),  append = TRUE)

POP[, grep("_sc20", names(POP), value = T) := NULL]
rm(out, nam, tt, x, RF)

...
# table 8
# BMI 
POP[ssb.tax.effect.mc, on = c("age", "qimd"), bmival_sc12a := bmival + effect]
POP[!between(age, 16, 84) | year < intervention_year2, bmival_sc12a := bmival]
setkey(POP, id, year) # Do not remove
POP[, bmival.cvdlag := shift_byid(bmival_sc12a, cvd.lag.mc, 20, id)] # lag exposure
...
# FV 
POP[, porftvg_sc12a := porftvg]
POP[between(age, 16, 84) & year >= intervention_year2 & rbinom(.N, 1, 0.5) == 1L,
    porftvg_sc12a := as.integer(bound(porftvg_sc12a + 1L, 0, 8))]
setkey(POP, id, year) # Do not remove
POP[, porftvg.cvdlag := shift_byid(porftvg_sc12a, cvd.lag.mc, 0, id)] # lag exposure
...
#SBP
POP[salt.reform.effect.mc, on = c("year", "agegroup", "sex", "qimd"), omsysval_sc12a := omsysval + i.effect] 
POP[!between(age, 20, 84) | year < intervention_year2, omsysval_sc12a := omsysval] # 0.8 mmHg reduction from salt policies
...
set(POP, NULL, "eligible_sc12a", 0L)
...
set(POP, NULL, "invited_sc12a", 0L)
set(POP, NULL, "dice_coverage",  dice(nrow(POP)))
setkey(POP, id, year) # Do not remove
POP[eligible_sc12a == 1 & year >= intervention_year, 
    invited_sc12a := HC_coverage(id, coverage, dice_coverage)]

# Estimate uptake
set(POP, NULL, "participated_sc12a", 0L)
...
POP[invited_sc12a == 1, uptake := fcompress(uptake, .N * hc_annual_uptake), by=year]# Compress weights to givel reach uptake level
POP[invited_sc12a == 1, participated_sc12a := rbinom(.N, 1, uptake)]
...
POP[participated_sc12a  == 1,  statins_uptake := bound(prescription_uptake * .N / count_if(cholval >= 5)), by = .(qimd, qrisk_grp)] # adjust weights to reflect only those with high cholesterol
POP[participated_sc12a  == 1 & cholval >= 5,
...
POP[participated_sc12a  == 1,  antihtn_uptake := bound(prescription_uptake * .N / count_if(omsysval >= 135)), by = .(qimd, qrisk_grp)] # adjust weights to reflect only those with high SBP
POP[participated_sc12a  == 1 & omsysval_sc12a >= 135,
    antihtn_uptake := fcompress(normalise(omsysval_sc12a, na.rm = T), sum(antihtn_uptake)),  
    by = .(qimd, qrisk_grp)] # give higher probabilities to those with higher SBP  

set(POP, NULL, "statins_prescribed_sc12a", 0L)
POP[participated_sc12a  == 1 & cholval >= 5,  statins_prescribed_sc12a := rbinom(.N, 1, statins_uptake)]
set(POP, NULL, "statins_taken_sc12a", 0L)
POP[statins_prescribed_sc12a  == 1,  statins_taken_sc12a := rbinom(.N, 1, persistence.mc)]

set(POP, NULL, "antihtn_prescribed_sc12a", 0L)
POP[participated_sc12a  == 1 & omsysval_sc12a >= 135,  antihtn_prescribed_sc12a := rbinom(.N, 1, antihtn_uptake)]
set(POP, NULL, "antihtn_taken_sc12a", 0L)
POP[antihtn_prescribed_sc12a  == 1,  antihtn_taken_sc12a := rbinom(.N, 1, persistence.mc)]

set(POP, NULL, "participation_effect_sc12a", 0L)
setkey(POP, id, year) # Do not remove
POP[, participation_effect_sc12a := HC_effect(id, participated_sc12a, invited_sc12a)] # Effect lasts until next invitation
POP[, statins_taken_sc12a := med_effect(id, statins_taken_sc12a, participation_effect_sc12a)] # Effect lasts until next invitation
POP[, antihtn_taken_sc12a := med_effect(id, antihtn_taken_sc12a, participation_effect_sc12a)] # Effect lasts until next invitation
#View(POP[year >= intervention_year& eligible_sc12a == 1, .(id, year, invited_sc12a, participated_sc12a, participation_effect_sc12a, statins_prescribed_sc12a, statins_taken_sc12a)])

# Estimate effect
#cholesterol
POP[, cholval_sc12a := cholval]
POP[statins_taken_sc12a  == 1,  cholval_sc12a := cholval * (1 - atorv.eff.mc * adherence.mc)]
setkey(POP, id, year) # Do not remove
POP[, cholval.cvdlag := shift_byid(cholval_sc12a, cvd.lag.mc, 3.8, id)] # lag exposure
#SBP
POP[antihtn_taken_sc12a  == 1,  omsysval_sc12a := omsysval_sc12a - (omsysval_sc12a - 135) * adherence.mc] # Assume that antihtn medication can potentialy achieve sbp 135 for all
POP[omsysval < omsysval_sc12a, omsysval_sc12a := omsysval]
setkey(POP, id, year) # Do not remove
POP[, omsysval.cvdlag := shift_byid(omsysval_sc12a, cvd.lag.mc, 115, id)] # lag exposure

#recalculate diabetes prob DO NOT REMOVE
# Diab probabilities
POP[, bpmed := bpmed_init] #reset for each scenario. not needed for sc12a. Here for consistency
POP[antihtn_taken_sc12a == 1L, bpmed := 1L] # feedback loop. Antihtn meds increased prescription increases diab 
...
set(POP, NULL, "htn.incidence_sc12a", 0L)
POP[omsysval_sc12a > 135 | bpmed == 1, htn.incidence_sc12a := 6000L]
...
# Calculate incidence
setkey(POP, id, year)
POP[, diabtotr_sc12a := diabtotr_init]
set(POP, NULL, "chd.incidence_sc12a", 0L)
set(POP, NULL, "stroke.incidence_sc12a", 0L)
POP[year >= 0, c("diabtotr_sc12a", "chd.incidence_sc12a", "stroke.incidence_sc12a") :=
...
POP[, diabtotr_sc12a := factor_(diabtotr_sc12a)]

# Calculate mortality
set(POP, NULL, "death.tob.rr", 1)
POP[cigst1.cvdlag == "4", death.tob.rr := smokriskofdeath]
set(POP, NULL, "death.diab.rr", 1)
POP[, diabtotr.cvdlag := shift_byid(diabtotr_sc12a, cvd.lag.mc, 1L, id, 1:2)] # lag exposure
...
POP[, `:=` (chd.mortality_sc12a = 0L,
            stroke.mortality_sc12a = 0L,
            other.mortality_sc12a = 0L)]
setkey(POP, id, year)
POP[year >= 0 & chd.incidence_sc12a > 0,
    chd.mortality_sc12a := mortality_events(pr_chd_death, dice_chd_death, id, -20L)]
POP[year >= 0 & stroke.incidence_sc12a > 0,
    stroke.mortality_sc12a := mortality_events(pr_stroke_death, dice_stroke_death,
                                             id, -3L)]
POP[year >= 0,
    other.mortality_sc12a := mortality_events(pr_other_death, dice_other_death,
                                            id, -100L)]
...
fwrite_safe(summary_RF_fn(RF, nam, "sc12a"),         output.dir("sc_RF.csv"),      append = TRUE)
fwrite_safe(summary_RF_fn(RF, nam, "sc12a", "qimd"), output.dir("sc_RF_qimd.csv"), append = TRUE)
fwrite_safe(summary_RF_fn(RF, nam, "sc12a", "sex"),  output.dir("sc_RF_sex.csv"),  append = TRUE)
fwrite_safe(summary_RF_fn(RF, nam, "sc12a", "age"),  output.dir("sc_RF_age.csv"),  append = TRUE)

# shape output
out <- mask_age(out)

fwrite_safe(summary_fn(out, nam, "sc12a"),         output.dir("sc_results.csv"),      append = TRUE)
fwrite_safe(summary_fn(out, nam, "sc12a", "qimd"), output.dir("sc_results_qimd.csv"), append = TRUE)
fwrite_safe(summary_fn(out, nam, "sc12a", "sex"),  output.dir("sc_results_sex.csv"),  append = TRUE)
fwrite_safe(summary_fn(out, nam, "sc12a", "age"),  output.dir("sc_results_age.csv"),  append = TRUE)

POP[, grep("_sc12a", names(POP), value = T) := NULL]
rm(out, nam, tt, x, RF)
